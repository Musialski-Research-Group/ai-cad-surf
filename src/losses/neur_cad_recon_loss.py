...
import utils
from .term import eikonal_loss, latent_rg_loss, gaussian_curvature


class NeurCADReconLoss(nn.Module):
    ...
    def forward(self, output_pred: dict, gt: dict):
        ...
        if self.use_morse:
            ...
            if self.div_type == 'l1':
                morse_loss = gaussian_curvature(nonmnfld_hessian_term, morse_nonmnfld_grad, use_DT=self.use_DT)

                if self.bidirectional_morse:
                    morse_mnfld = gaussian_curvature(mnfld_hessian_term, mnfld_grad, use_DT=self.use_DT)
